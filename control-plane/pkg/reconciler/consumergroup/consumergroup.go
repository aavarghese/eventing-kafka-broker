...
func (r *Reconciler) reconcileScaledObject(ctx context.Context, expectedScaledObject *kedav1alpha1.ScaledObject, obj metav1.Object) error {
	...
	if apierrors.IsNotFound(err) {
		...
		if err != nil {
			return reckafka.ScaleObjectDeploymentFailed(expectedScaledObject.Namespace, expectedScaledObject.Name, err)
		}
		return reckafka.ScaleObjectCreated(scaledObject.Namespace, scaledObject.Name)
	} else if err != nil {
		...
		return reckafka.ScaleObjectUpdated(scaledObject.Namespace, scaledObject.Name)
	} else {
